...
import type { ErpPending } from "@/lib/cartera-data";
...
export async function PendingDetail({ row }: { row: ErpPending }) {
  ...
  return (
    <div>
      ...
      <Field label="Documento ERP" value={row.doc_erp} />
      ...
      <div className="mt-4 rounded-md border border-line bg-parchment p-4">
        {emailMatch ? (
          <>
            ...
            <p className="text-sm text-graphite">Proveedor en correo: {emailMatch.proveedor_correo}</p>
            <p className="num text-sm text-graphite">Valor en correo: {formatCurrency(emailMatch.valor_total_correo)}</p>
            {Number(emailMatch.valor_total_correo) !== Number(row.valor_total_erp) && (
              <p className="mt-1 text-sm font-semibold text-orange">
                Diferencia de valor: {formatCurrency(Math.abs(Number(emailMatch.valor_total_correo) - Number(row.valor_total_erp)))}
              </p>
            )}
          </>
        ) : (
          <p className="flex items-center gap-2 text-sm font-semibold text-red-deep">
            <AlertCircle size={16} />
            Sin factura en correo — verificar recepción con el proveedor.
          </p>
        )}
      </div>
    </div>
