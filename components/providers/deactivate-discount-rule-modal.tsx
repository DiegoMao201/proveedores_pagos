...
import { deactivateDiscountRule } from "@/lib/discount-rule-actions";
import type { DiscountRuleFull } from "@/lib/discount-rule-data";

export function DeactivateDiscountRuleModal({ providerId, rule }: { providerId: number; rule: DiscountRuleFull }) {
  ...
  function handleConfirm() {
    startTransition(async () => {
      try {
        await deactivateDiscountRule(rule.id, providerId, motivo);
        showToast({ kind: "success", message: "Peldaño desactivado. Aplica a facturas nuevas desde mañana." });
        setOpen(false);
        router.refresh();
      } catch (e) {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="¿Desactivar este peldaño?" width={440}>
        <div className="flex flex-col gap-3">
          <p className="text-stone" style={{ fontSize: 11 }}>
            Peldaño: <strong className="text-ink">{rule.nombre_regla ?? `${(rule.tasa_descuento * 100).toFixed(1)}% a ${rule.dias_max} días`}</strong>
            <br />
            Vigente desde: {rule.valid_from}
          </p>
          <div className="rounded-md border border-yellow bg-cream-soft px-3 py-2" style={{ fontSize: 10.5 }}>
            Facturas emitidas hasta hoy seguirán usando esta regla al calcular su descuento (versionado estricto). Solo
            dejará de aplicar a facturas nuevas.
          </div>
          <textarea
          ...
          <div className="flex justify-end gap-2">
            ...
            >
              {pending ? "Desactivando…" : "Desactivar peldaño"}
            </button>
          </div>
        </div>
      </Modal>
